...
class Prober
	...
	PROBER_VERSION = "$Id: prober.rb,v 1.10 2010/09/19 21:26:32 ethan Exp $".split(" ").at(2).to_f

	# URI to fetch current controller port from
	CONTROLLER_INFO="http://revtr.cs.washington.edu/vps/isolation_registrar.txt"
	...
	def log(msg,except=nil)
        ...
        # Better!: http://ruby-doc.org/stdlib/libdoc/logger/rdoc/index.html
	end
	...
	# open holes in the firewall to receive spoofed probes through
	def punch_holes(targs)
		self.log "Punching #{targs.length} holes"
		fn=create_target_file{|targ_file| targ_file.puts targs.join("\n")}
		results=`sudo #{@proberoute_dir}/spoofprobe #{Prober::PROBING_THREADS} #{fn} #{@device}`
		self.remove_files(fn)
		return results
	end
	...
	def spoof_rr(probes)
		...
		`sudo #{@proberoute_dir}/rrspoof #{Prober::PROBING_THREADS} #{fn} #{@device}`
		self.remove_files(fn)
	end
	...
	def spoof_ts(probes)
		...
		`sudo #{@proberoute_dir}/tsprespec-spoof #{Prober::PROBING_THREADS} #{fn} #{@device}`
		self.remove_files(fn)
	end

	# probes is hash[receiver] -> [tr1, tr2, ...]
	# tr is either [dst, id], [dst, id, start_ttl], or [dst, id, start_ttl, end_ttl]
	# if ttls aren't given, will do 1..30
	# id is the spoofer ID to put into the probes
	# must be at most 11 bits
	def spoof_tr(probes)
		fn = create_target_file do |targf|
			probes.keys.each do |rec|
				trs = probes[rec]
				self.log "sending spoofed traceroute, as #{rec}, targs " + trs.map{|x| x.join(",")}.join(" ") 
				trs.each do |tr|
                    dst = tr[0]
                    id = tr[1]
		            if id>2047  or id<0
					...
					when 2
					    start,finish = 1,30
					when 3
					    start,finish = tr[2],30
					else
				        start,finish = tr[2],tr[3]
				    end
					...
					(start..finish).each do |ttl|
						targf.puts "#{dst} #{id} #{ttl} #{rec}"
                    end
                end
		    end	
        end
		system "sudo #{@proberoute_dir}/pingspoof #{Prober::PROBING_THREADS} #{fn} #{@device}"
		self.remove_files(fn) # XXX
	end

	def receive_spoofed_probes(type=:rrspoof)
		uid = generate_id()
		...
		Thread.new{`sudo #{@proberoute_dir}/#{type}-recv #{@device} #{fn}`}
		return fn
	end

	def receive_spoofed_rr
		receive_spoofed_probes(:rrspoof)
	end

	def receive_spoofed_ts
		receive_spoofed_probes(:tsprespec)
	end

    def receive_spoofed_tr
		receive_spoofed_probes(:pingspoof)
    end

	# kill receivers and retrieve results
	# does killall at the moment - makes it not safe to make multiple receive
	# calls at once
	def kill_and_retrieve(fid)
		`sudo killall -9 -e tsprespec-recv 1> /dev/null 2>&1`
		`sudo killall -9 -e rrspoof-recv 1> /dev/null 2>&1`
		`sudo killall -9 -e pingspoof-recv 1> /dev/null 2>&1`
		f=nil
		begin
			...
			return [probes,Set.new(sources).to_a]
