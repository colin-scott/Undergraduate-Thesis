...
describe "stuff" do
    before(:all) do
        @source = "toil.cs.washington.edu"
        @source_ip = "128.208.4.244"
        @ping_receiver = "crash.cs.washington.edu"
        @trace_receiver = "riot.cs.washington.edu"
        @test_target = "74.125.127.106"
        @mock_targets = [@test_target]
        @mock_hosts = [@source,@ping_receiver,@trace_receiver]

        @mock_ping_results = [["#{@test_target} 52 58 7.992000 32654",@source]]
        @mock_ping_controller_return = [@mock_ping_results,[],[],[]] 
        ...
        ## TODO: lower case h?
        #@trace_binary = @trace_hex.pack("H" * @trace_hex.size)
        @trace_binary = IO.read("fixtures/toil.cs.washington.edu.trace.out")

        # Mock spoofed traceroute
        @mock_trace_results = [[@trace_binary,@source]]
        @mock_trace_controller_return = [@mock_trace_results,[],[],[]]

        # For spoofed pings, controller returns  [[probes,receiver],...]
        # probes is [ascii output, @sources]
        @mock_spoofed_ping_results = [[["#{@test_target} 1 10 #{@source_ip}",[@source]],@ping_receiver]]
        @mock_spoofed_ping_controller_return = [@mock_spoofed_ping_results,[],[],[]]

        # TTL 1 and 2, spoofer id 0
        @mock_spoofed_trace_results = [[["#{@test_target} 0 1 0\n#{@test_target} 0 2 0",[@source]],@trace_receiver]]
        @mock_spoofed_trace_controller_return = [@mock_spoofed_trace_results,[],[],[]]

        class MockController
            # Fuck, cludgy... double, y u no work?
            def initialize(*args)
                @mock_hosts,@mock_ping_controller_return,@mock_trace_controller_return,@mock_spoofed_ping_controller_return,@mock_spoofed_trace_controller_return,@ping_receiver = *args
            end
            ...
            def spoof_tr(receiver2spoofer2targets,&block)
        ...
        ProbeController::set_server(:controller,MockController.new(@mock_hosts,@mock_ping_controller_return,@mock_trace_controller_return,@mock_spoofed_ping_controller_return,@mock_spoofed_trace_controller_return,@ping_receiver))
    end
