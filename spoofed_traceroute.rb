...
module SpoofedTR
    ...
    def SpoofedTR::sendProbes(hostname, dests, receivers, controller)
        ...
        controller.log.warn "SpoofedTR::sendProbes(), source #{hostname}, dests #{dests} receivers are #{receivers.join(',')}"
        ...
        controller.log.warn "SpoofedTR::sendProbes(), receiver2spoofer2targets: #{receiver2spoofer2targets.inspect}"
        results,unsuccessful_receivers,privates,blacklisted = controller.spoof_tr(receiver2spoofer2targets)

        controller.log.warn "SpoofedTR::sendProbes(#{hostname} #{dests.inspect}), results,unsuccessful_receivers,privates,blacklisted"
        controller.log.warn "#{results.inspect},#{unsuccessful_receivers.inspect},#{privates.inspect},#{blacklisted.inspect}"
    ...
    def SpoofedTR::parse_path(results, id2dest, controller)
        controller.log.warn "parse_path(), results: #{results.inspect}"
        controller.log.warn "parse_path(), id2dest: #{id2dest.inspect}"
        ...
        controller.log.warn "parse_path(), id2dest #{id2dest.inspect}"
        controller.log.warn "parse_path(), dest2ttl2rtrs before merge #{dest2ttl2rtrs.inspect}"
        ...
        controller.log.warn "parse_path(), dest2ttl2rtrs after merge #{dest2ttl2rtrs.inspect}"
        ...
        controller.log.warn "parse_path(), dest2ttl2rtrs to arrays: #{dest2ttl2rtrs.inspect}"
        ...
        dest2ttl2rtrs.keys.each do |dest|
            ...
            controller.log.warn "parse_path(#{dest}): sortedttlrtrs: #{sortedttlrtrs.inspect}"
            ...
            controller.log.warn "parse_path(#{dest}): sortedttlrtrs after removing dest: #{sortedttlrtrs.inspect}"
            ...
            controller.log.warn "parse path, sortedttlrtrs.size: #{sortedttlrtrs.size}"
            if sortedttlrtrs.size > 2
                controller.log.warn "parse path, sortedttlrtrs.last ttl: #{sortedttlrtrs[-1][0].to_i}"
                controller.log.warn "parse path, sortedttlrtrs.last ttl: #{sortedttlrtrs[-2][0].to_i}"
            end
        ...
        controller.log.warn "parse_path(), dest2sortedttlrtrs converting to arrays #{dest2sortedttlrtrs.inspect}"
